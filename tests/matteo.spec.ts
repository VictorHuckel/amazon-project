...
test.describe('Tests Amazon', () => {
  ...
  test('Supprimer tous les produits du panier', async ({page}) => {
    ...
    await amazonPage.removeAllProductsFromCart();

    // Vérifier que le panier est vide
    await amazonPage.goToCart();
    const cartItems = await page.locator('.sc-product-title').count();
    expect(cartItems).toBe(0); // Vérifie que le panier est vide
  });
